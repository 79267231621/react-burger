...
export default function BurgerIngredients () {
    ...
    const { ingredients, groups, ingredientsRequest } = useSelector(store => store.ingredients);
    ...
    const onClickTab = (value) => {
        let index = groups.findIndex(groupe => groupe.value === value);
        ...
        <section className="col col-50 p-5">
            ...
            <div style={{ display: 'flex' }} className="mb-10 ">
                {groups.map((group, index)=>(
                    <Tab value={group.value} active={tab === group.value} onClick={onClickTab} key={index}>
                        {group.name}
                    </Tab>
                ))}
            </div>
